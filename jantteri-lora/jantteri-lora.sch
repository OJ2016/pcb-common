EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 6
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 3650 1450 550  400 
U 6115D0E4
F0 "AMS1117-3.3" 50
F1 "AMS1117-3.3.sch" 50
F2 "VIN" I L 3650 1550 50 
F3 "VOUT" I R 4200 1550 50 
F4 "GND" I L 3650 1650 50 
$EndSheet
$Sheet
S 8150 1250 550  1000
U 6112EDA0
F0 "RFM95" 50
F1 "RFM95.sch" 50
F2 "DIO0" I L 8150 1450 50 
F3 "GND" I R 8700 1450 50 
F4 "RST" I L 8150 1350 50 
F5 "NSS" I L 8150 1650 50 
F6 "MISO" I L 8150 1750 50 
F7 "MOSI" I L 8150 1850 50 
F8 "SCK" I L 8150 1950 50 
F9 "3V3" I R 8700 1350 50 
F10 "DIO1" I R 8700 1700 50 
F11 "DIO2" I R 8700 1800 50 
F12 "DIO3" I R 8700 1900 50 
F13 "DIO4" I R 8700 2000 50 
F14 "DIO5" I R 8700 2100 50 
$EndSheet
$Comp
L power:+3V3 #PWR08
U 1 1 61143103
P 5850 1850
F 0 "#PWR08" H 5850 1700 50  0001 C CNN
F 1 "+3V3" H 5865 2023 50  0000 C CNN
F 2 "" H 5850 1850 50  0001 C CNN
F 3 "" H 5850 1850 50  0001 C CNN
	1    5850 1850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR09
U 1 1 611435CD
P 5850 1950
F 0 "#PWR09" H 5850 1700 50  0001 C CNN
F 1 "GND" H 5855 1777 50  0000 C CNN
F 2 "" H 5850 1950 50  0001 C CNN
F 3 "" H 5850 1950 50  0001 C CNN
	1    5850 1950
	1    0    0    -1  
$EndComp
Wire Wire Line
	6100 1950 5850 1950
Wire Wire Line
	5850 1850 6100 1850
$Comp
L power:GND #PWR07
U 1 1 611483EC
P 5050 2500
F 0 "#PWR07" H 5050 2250 50  0001 C CNN
F 1 "GND" H 5055 2327 50  0000 C CNN
F 2 "" H 5050 2500 50  0001 C CNN
F 3 "" H 5050 2500 50  0001 C CNN
	1    5050 2500
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR010
U 1 1 61148B21
P 7950 2900
F 0 "#PWR010" H 7950 2650 50  0001 C CNN
F 1 "GND" H 7955 2727 50  0000 C CNN
F 2 "" H 7950 2900 50  0001 C CNN
F 3 "" H 7950 2900 50  0001 C CNN
	1    7950 2900
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR06
U 1 1 61148EAA
P 5050 2400
F 0 "#PWR06" H 5050 2250 50  0001 C CNN
F 1 "+3V3" H 5065 2573 50  0000 C CNN
F 2 "" H 5050 2400 50  0001 C CNN
F 3 "" H 5050 2400 50  0001 C CNN
	1    5050 2400
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR011
U 1 1 6114B431
P 8900 1350
F 0 "#PWR011" H 8900 1200 50  0001 C CNN
F 1 "+3V3" H 8915 1523 50  0000 C CNN
F 2 "" H 8900 1350 50  0001 C CNN
F 3 "" H 8900 1350 50  0001 C CNN
	1    8900 1350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR012
U 1 1 6114B78E
P 8900 1450
F 0 "#PWR012" H 8900 1200 50  0001 C CNN
F 1 "GND" H 8905 1277 50  0000 C CNN
F 2 "" H 8900 1450 50  0001 C CNN
F 3 "" H 8900 1450 50  0001 C CNN
	1    8900 1450
	1    0    0    -1  
$EndComp
Wire Wire Line
	8700 1450 8900 1450
Wire Wire Line
	8700 1350 8900 1350
$Comp
L power:GND #PWR03
U 1 1 6114C811
P 3550 1650
F 0 "#PWR03" H 3550 1400 50  0001 C CNN
F 1 "GND" H 3555 1477 50  0000 C CNN
F 2 "" H 3550 1650 50  0001 C CNN
F 3 "" H 3550 1650 50  0001 C CNN
	1    3550 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	3550 1650 3650 1650
$Comp
L power:+3V3 #PWR05
U 1 1 6114D52E
P 4550 1550
F 0 "#PWR05" H 4550 1400 50  0001 C CNN
F 1 "+3V3" H 4565 1723 50  0000 C CNN
F 2 "" H 4550 1550 50  0001 C CNN
F 3 "" H 4550 1550 50  0001 C CNN
	1    4550 1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4550 1550 4200 1550
$Sheet
S 8150 2650 650  300 
U 6114F8E6
F0 "jst-dbg" 50
F1 "jst-dbg.sch" 50
F2 "DBG_TX" I R 8800 2750 50 
F3 "DBG_RX" I R 8800 2850 50 
F4 "GND" I L 8150 2850 50 
F5 "VCC" I L 8150 2750 50 
$EndSheet
Wire Wire Line
	8150 2850 7950 2850
Wire Wire Line
	7950 2850 7950 2900
Text GLabel 8150 2750 0    50   Input ~ 0
USB-5V
$Comp
L Device:Ferrite_Bead FB?
U 1 1 6115E9C4
P 2750 1550
AR Path="/611583F8/6115E9C4" Ref="FB?"  Part="1" 
AR Path="/6115E9C4" Ref="FB1"  Part="1" 
F 0 "FB1" V 2476 1550 50  0000 C CNN
F 1 "GZ2012D101TF" V 2567 1550 50  0000 C CNN
F 2 "Inductor_SMD:L_0805_2012Metric" V 2680 1550 50  0001 C CNN
F 3 "~" H 2750 1550 50  0001 C CNN
F 4 "C1015" V 2750 1550 50  0001 C CNN "LCSC"
	1    2750 1550
	0    1    1    0   
$EndComp
Text Notes 650  1050 0    50   ~ 0
5V-IN disables USB power input. If no 5V-IN connected, USB-5V is used 
Wire Wire Line
	2900 1550 3650 1550
Text GLabel 6100 3450 0    50   Input ~ 0
SPI1_SCK
Text GLabel 6100 3950 0    50   Input ~ 0
SPI1_MISO
Text GLabel 6100 3550 0    50   Input ~ 0
SPI1_MOSI
Text GLabel 8150 1750 0    50   Input ~ 0
SPI1_MISO
Text GLabel 8150 1850 0    50   Input ~ 0
SPI1_MOSI
Text GLabel 8150 1950 0    50   Input ~ 0
SPI1_SCK
Text GLabel 8150 1350 0    50   Input ~ 0
RFM_RST
Text GLabel 8150 1450 0    50   Input ~ 0
RFM_DIO0
Text GLabel 8150 1650 0    50   Input ~ 0
RFM_NSS
Text GLabel 6100 3650 0    50   Input ~ 0
RFM_RST
Text GLabel 6900 2550 2    50   Input ~ 0
RFM_DIO0
Text GLabel 6100 3850 0    50   Input ~ 0
RFM_NSS
$Sheet
S 6100 1750 800  3050
U 6116447D
F0 "STM32G030C8T6" 50
F1 "STM32G030C8T6.sch" 50
F2 "3V3" I L 6100 1850 50 
F3 "GND" I L 6100 1950 50 
F4 "NRST" I L 6100 2400 50 
F5 "SWDIO" I L 6100 2500 50 
F6 "SWCLK" I L 6100 2600 50 
F7 "PB13" I R 6900 3150 50 
F8 "PB14" I R 6900 3250 50 
F9 "PB15" I R 6900 3350 50 
F10 "PA8" I L 6100 4150 50 
F11 "PC6" I R 6900 4500 50 
F12 "PC7" I R 6900 4650 50 
F13 "PA11" I L 6100 4450 50 
F14 "PA12" I L 6100 4550 50 
F15 "PA15" I L 6100 4650 50 
F16 "PD0" I R 6900 3750 50 
F17 "PD1" I R 6900 3850 50 
F18 "PD2" I R 6900 3950 50 
F19 "PD3" I R 6900 4050 50 
F20 "PB3" I R 6900 2150 50 
F21 "PB4" I R 6900 2250 50 
F22 "PB5" I R 6900 2350 50 
F23 "PB8" I R 6900 2650 50 
F24 "PB9" I R 6900 2750 50 
F25 "PA0" I L 6100 3350 50 
F26 "PA1" I L 6100 3450 50 
F27 "PA2" I L 6100 3550 50 
F28 "PA3" I L 6100 3650 50 
F29 "PA4" I L 6100 3750 50 
F30 "PA5" I L 6100 3850 50 
F31 "PA6" I L 6100 3950 50 
F32 "PA7" I L 6100 4050 50 
F33 "PB0" I R 6900 1850 50 
F34 "PB1" I R 6900 1950 50 
F35 "PB2" I R 6900 2050 50 
F36 "PB10" I R 6900 2850 50 
F37 "PB11" I R 6900 2950 50 
F38 "PB12" I R 6900 3050 50 
F39 "PA9" I L 6100 4250 50 
F40 "PA10" I L 6100 4350 50 
F41 "PB7" I R 6900 2550 50 
F42 "PB6" I R 6900 2450 50 
$EndSheet
Text GLabel 8800 2750 2    50   Input ~ 0
DBG_TX
Text GLabel 8800 2850 2    50   Input ~ 0
DBG_RX
Text GLabel 6100 4250 0    50   Input ~ 0
DBG_TX
Text GLabel 6100 4350 0    50   Input ~ 0
DBG_RX
$Comp
L Transistor_FET:Si2319CDS Q?
U 1 1 628A2F57
P 1800 1650
AR Path="/611583F8/628A2F57" Ref="Q?"  Part="1" 
AR Path="/628A2F57" Ref="Q1"  Part="1" 
AR Path="/6117A90D/628A2F57" Ref="Q?"  Part="1" 
AR Path="/611A8D7A/628A2F57" Ref="Q?"  Part="1" 
F 0 "Q1" V 2142 1650 50  0000 C CNN
F 1 "IRLML6402TRPBF" V 2051 1650 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 2000 1575 50  0001 L CIN
F 3 "http://www.vishay.com/docs/66709/si2319cd.pdf" H 1800 1650 50  0001 L CNN
F 4 "C2593" V 1800 1650 50  0001 C CNN "LCSC"
	1    1800 1650
	0    -1   -1   0   
$EndComp
Wire Wire Line
	1250 1550 1600 1550
Text GLabel 1250 1550 0    50   Input ~ 0
USB-5V
Wire Wire Line
	2000 1550 2600 1550
$Comp
L power:GND #PWR0101
U 1 1 628A63B0
P 1800 1850
F 0 "#PWR0101" H 1800 1600 50  0001 C CNN
F 1 "GND" H 1805 1677 50  0000 C CNN
F 2 "" H 1800 1850 50  0001 C CNN
F 3 "" H 1800 1850 50  0001 C CNN
	1    1800 1850
	1    0    0    -1  
$EndComp
$Sheet
S 5200 2300 650  450 
U 628A7B72
F0 "TC2030" 50
F1 "TC2030ARM.sch" 50
F2 "3V3" I L 5200 2400 50 
F3 "GND" I L 5200 2500 50 
F4 "SWDIO" I R 5850 2500 50 
F5 "SWCLK" I R 5850 2600 50 
F6 "SWO" I R 5850 2700 50 
F7 "NRST" I R 5850 2400 50 
$EndSheet
Wire Wire Line
	5050 2400 5200 2400
Wire Wire Line
	5050 2500 5200 2500
Wire Wire Line
	5850 2400 6100 2400
Wire Wire Line
	5850 2500 6100 2500
Wire Wire Line
	5850 2600 6100 2600
NoConn ~ 5850 2700
Text GLabel 8700 1700 2    50   Input ~ 0
RFM_DIO1
Text GLabel 8700 1800 2    50   Input ~ 0
RFM_DIO2
Text GLabel 8700 1900 2    50   Input ~ 0
RFM_DIO3
Text GLabel 8700 2000 2    50   Input ~ 0
RFM_DIO4
Text GLabel 8700 2100 2    50   Input ~ 0
RFM_DIO5
Text GLabel 6900 2450 2    50   Input ~ 0
RFM_DIO1
Text GLabel 6900 2350 2    50   Input ~ 0
RFM_DIO2
Text GLabel 6900 2750 2    50   Input ~ 0
RFM_DIO3
Text GLabel 6900 2650 2    50   Input ~ 0
RFM_DIO4
Text GLabel 6900 1850 2    50   Input ~ 0
RFM_DIO5
$EndSCHEMATC
